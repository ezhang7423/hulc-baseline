...
class ConcatBlock(nn.Module):
    def __init__(self, dim, with_residual=True, with_batchnorm=True):
        ...
        self.res_block = ResidualBlock(dim, with_residual=with_residual, with_batchnorm=with_batchnorm)
...
def build_stem(feature_dim, module_dim, num_layers=2, with_batchnorm=True, kernel_size=3, stride=1, padding=None):
    layers = []
    ...
    for i in range(num_layers):
        layers.append(nn.Conv2d(prev_dim, module_dim, kernel_size=kernel_size, stride=stride, padding=padding))
        if with_batchnorm:
