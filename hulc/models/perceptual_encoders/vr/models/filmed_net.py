...
class FiLMedNet(nn.Module):
    def __init__(
        ...
        for fn_num in range(self.num_modules):
            with_cond = self.condition_pattern[self.module_num_layers * fn_num : self.module_num_layers * (fn_num + 1)]
            mod = self.block(
                module_dim,
    ...
    def forward(self, x, film, save_activations=False):
        ...
        module_inputs = Variable(
            torch.zeros(feats.size()).unsqueeze(1).expand(N, self.num_modules, self.module_dim, H, W)
        ).type(torch.cuda.FloatTensor)
        module_inputs[:, 0] = feats
        for fn_num in range(self.num_modules):
            if self.condition_method == "concat":
                layer_output = self.function_modules[fn_num](
                    module_inputs[:, fn_num], extra_channels=batch_coords, cond_maps=cond_maps[:, fn_num]
                )
            else:
                layer_output = self.function_modules[fn_num](
                    module_inputs[:, fn_num], gammas[:, fn_num, :], betas[:, fn_num, :], batch_coords
                )
...
class FiLMedResBlock(nn.Module):
    def __init__(
        ...
        if (self.condition_method == "relu-film" or self.condition_method == "block-output-film") and self.with_cond[0]:
            self.film = FiLM()
